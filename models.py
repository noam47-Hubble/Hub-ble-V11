...
class Card(db.Model):
    ...
    def public_dict(self):
        """Only the fields this card's owner has marked visible."""
        visible_photos = [p.url for p in sorted(self.photos, key=lambda p: p.order) if p.is_visible]
        out = {
            "id": self.id, "type": self.card_type,
            "photo_url": visible_photos[0] if visible_photos else self.photo_url,
    ...
    def full_dict(self):
        ...
        out = {
            ...
            "photos": [
                {"id": p.id, "url": p.url, "is_visible": p.is_visible}
...
class CardPhoto(db.Model):
    """One photo in a card's gallery. Independent visibility per photo, so
    the owner can upload several and choose which ones actually show up on
    their public card (per the spec: a 'show in profile' toggle per photo)."""
    id = db.Column(db.String, primary_key=True, default=gen_id)
...
class HubBlock(db.Model):
    ...
    (title + free text) rather than one table per module type: the
