  ...
  conversations.forEach(c => {
    ...
    row.innerHTML = `
      <div class="card-row-info">
        <div class="card-row-name">${c.from_name} ${c.is_green_now ? '<span class="presence-dot" style="color:#1a7f37">🟢 זמין</span>' : '<span class="hint" style="display:inline">לא זמין כרגע</span>'}</div>
...
document.getElementById('btn-open-hub').addEventListener('click', async () => {
  ...
  openHubForm.reset();
  document.getElementById('professional-hub-fields').classList.add('hidden');
  document.getElementById('hub-logo-preview').classList.add('hidden');
  openHubLogoDataUrl = '';
  document.getElementById('template-id-input').value = 'corporate_classic';
  renderTemplatePicker('corporate_classic');
...
document.getElementById('open-hub-type-select').addEventListener('change', e => {
  document.getElementById('professional-hub-fields').classList.toggle('hidden', e.target.value !== 'professional');
});

// ---- design template picker (5 pre-built templates, spec §4.2) ----

const HUB_TEMPLATES = [
  { id: 'corporate_classic', name: 'קלאסי-ארגוני', colors: ['#2f5fa8', '#eef3fa'] },
  { id: 'community_dynamic', name: 'קהילתי-דינמי', colors: ['#3a9d5d', '#f2994a'] },
  { id: 'creator_purple', name: 'יצירתי-סגול', colors: ['#8b5cf6', '#f3e8ff'] },
  { id: 'minimalist_dark', name: 'מינימליסטי כהה', colors: ['#12141f', '#e8a33d'] },
  { id: 'logistics_red', name: 'לוגיסטי-אדום', colors: ['#c0392b', '#fdecea'] },
];

function renderTemplatePicker(selectedId) {
  ...
  HUB_TEMPLATES.forEach(t => {
    ...
    swatch.addEventListener('click', () => {
      document.getElementById('template-id-input').value = t.id;
...
let openHubLogoDataUrl = '';
document.getElementById('hub-logo-input').addEventListener('change', e => {
  const file = e.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = () => {
    openHubLogoDataUrl = reader.result;
    const preview = document.getElementById('hub-logo-preview');
    preview.src = openHubLogoDataUrl;
...
openHubForm.addEventListener('submit', async e => {
  ...
  payload.auto_approve = openHubForm.elements['auto_approve'].checked;
  payload.publish_now = openHubForm.elements['publish_now'].checked;
  if (openHubLogoDataUrl) payload.logo_url = openHubLogoDataUrl;
...
function ensureSocket() {
  ...
  function renderConferenceBanner(hub) {
    ...
    document.getElementById('chat-tab-directory').classList.toggle('hidden', !isProfessional);
    document.getElementById('chat-tab-program').classList.toggle('hidden', !isProfessional);
    ...
    logo.classList.toggle('hidden', !hub.logo_url);
    if (hub.logo_url) logo.src = hub.logo_url;
    document.getElementById('conference-tagline').textContent = hub.tagline || hub.name;
    const metaParts = [hub.location, hub.event_dates].filter(Boolean);
  ...
  socket.on('presence_list', data => {
    ...
    if (me) {
      document.getElementById('status-green').classList.toggle('active', me.status === 'green');
      document.getElementById('status-red').classList.toggle('active', me.status === 'red');
  ...
  socket.on('private_message', msg => {
    ...
    if (msg.from_user_id !== myUserId) {
      const viewingThisThread =
        (privateTargetUserId === otherId && !document.getElementById('screen-hub').classList.contains('hidden')) ||
  ...
  socket.on('blocks_updated', ({ hub_id }) => {
    if (hub_id === currentHubId && !document.getElementById('program-panel').classList.contains('hidden')) {
      loadProgramBlocks();
    }
  });
...
function renderParticipants(presence) {
  ...
  presence.forEach(p => {
    ...
    row.innerHTML = `
      <span class="presence-status ${p.status}"></span>
...
document.getElementById('status-green').addEventListener('click', () => socket && socket.emit('set_status', { status: 'green' }));
document.getElementById('status-red').addEventListener('click', () => socket && socket.emit('set_status', { status: 'red' }));
...
document.querySelectorAll('.chat-tab').forEach(tab => {
  tab.addEventListener('click', () => {
    ...
    document.getElementById('directory-panel').classList.toggle('hidden', tab.dataset.tab !== 'directory');
    document.getElementById('program-panel').classList.toggle('hidden', tab.dataset.tab !== 'program');
    if (tab.dataset.tab === 'program') loadProgramBlocks();
